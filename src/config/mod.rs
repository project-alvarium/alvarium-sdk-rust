...
mod make_config_tests {
    use super::{SdkInfo, StreamInfo, DemiaStreamsConfig, MqttStreamConfig};
    #[test]
    fn new_config() {
    ...
    fn demia_streams_config() {
        let config: StreamInfo = serde_json::from_slice(crate::DEMIA_TEST_CONFIG_BYTES.as_slice()).unwrap();
